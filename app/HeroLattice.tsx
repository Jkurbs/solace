...
function parseInk(color: string): Rgb {
...
function roleColor(role: Role): Rgb {
  switch (role) {
    ...
    case 'exploratory':
      return PALETTE.exploratory;
    case 'bridge':
      return PALETTE.bridge;
    case 'ephemeral':
      return PALETTE.ephemeral;
    default:
...
export default function HeroLattice({
  ...
  useEffect(() => {
    ...
    let baseInk = parseInk(getComputedStyle(mount).color);
    ...
    const draw = (timeSec: number, dt: number) => {
      ...
      // Fill the mount — Stripe-scale band uses a large canvas.
      const scale = minDim * 0.168;
      const perspective = minDim * 0.92;
      ...
      ctx.strokeStyle = rgba(mixRgb(ringCol, PALETTE.exploratory, 0.3), tickAlpha);
      for (let k = 0; k < 4; k++) {
