 ...
 *   - No self-referencing canvas draws (undefined behavior)
 *   - Every composite operation is paired with save/restore
 *   - All animation state resets on unmount
 */
...
const posturePalette: Record<HermesPublicPosture, Palette> = {
  DEPLOYED:      { r: 218, g: 165, b:  85 },
  SELECTIVE:     { r: 135, g: 206, b: 235 },
  DEFENSIVE:     { r: 176, g: 196, b: 222 },
  STANDING_DOWN: { r: 147, g: 112, b: 219 },
  RISK_OFF:      { r: 178, g: 102, b: 102 },
};

const defaultPalette: Palette = { r: 200, g: 195, b: 188 };
...
function lerpColor(a: Palette, b: Palette, t: number): Palette {
...
/* ── Grain ── */
function createGrainCanvas(): HTMLCanvasElement | null {
...
export default function HeroLattice({
  ...
  useEffect(() => {
    ...
    /* Color */
    let currentInk: Palette = { ...defaultPalette };
    let targetInk: Palette = posture ? { ...posturePalette[posture] } : { ...defaultPalette };
    let transitionT = 0;
    ...
    const resize = () => {
      ...
      const bw = Math.floor(width * dpr * 0.5);
      const bh = Math.floor(height * dpr * 0.5);
    ...
    const draw = (timeSec: number, dt: number) => {
      ...
      /* ── Bloom Pass (half-res, safe scratch blur) ── */
      ...
      /* Safe blur: draw to scratch, then back */
      blurCtx.clearRect(0, 0, blurScratch.width, blurScratch.height);
      blurCtx.filter = 'blur(6px)';
      blurCtx.drawImage(bloomCanvas, 0, 0);
      blurCtx.filter = 'none';

      bloomCtx.clearRect(0, 0, bloomCanvas.width, bloomCanvas.height);
      bloomCtx.drawImage(blurScratch, 0, 0);
