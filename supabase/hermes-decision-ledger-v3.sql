-- Hermes Decision Ledger V3 — row classes and open/close pairing.
-- Adds three UNHASHED metadata columns (the canonical hash covers an
-- enumerated field list, so existing hashes are untouched):
--   row_class:  'sealed' (at decision time) | 'backfill' (recorded after
--               outcome; labeled by a sealed system row) | 'system' (meta)
--   event_type: 'open' | 'close' | 'void' for path lifecycle rows
--   ref:        for close/void rows, the record_id of the open row
-- All three are write-once: settable while null, frozen after.
-- Run after hermes-decision-ledger-v2.sql.

alter table public.hermes_decision_ledger
  add column if not exists row_class text,
